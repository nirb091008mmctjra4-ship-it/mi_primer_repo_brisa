...
def abrir_empleados():
    ...
    empleados.geometry("700x600")
    empleados.configure(bg="#FFD700")  
    campos = ["id_empleado", "nombre_empleado", "cargo", "numero_contacto", "id_sucursal"]
    entradas = {}

    for i, texto in enumerate(campos):
        Tk.Label(empleados, text=texto, bg="#FFD700").grid(row=i, column=0, padx=10, pady=5, sticky="w")
    ...
    for i, (texto, cmd) in enumerate(botones):
        Tk.Button(empleados, text=texto, width=12, bg="#000000", fg="#FFD700", command=cmd).grid(row=4, column=i, padx=10, pady=10)
    ...
    Tk.Button(empleados, text="Regresar al Menú", width=20, bg="#000000", fg="#FFD700", command=lambda: [empleados.destroy(), menu.abrir_menu()]).grid(row=7, column=0, columnspan=4, pady=10)
