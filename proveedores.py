...
def abrir_proveedores():
    ...
    proveedores.configure(bg="#FFD700")  

    campos = ["id_proveedor", "nombre_proveedor", "numero_contacto", "e_mail"]
    entradas = {}

    for i, texto in enumerate(campos):
        Tk.Label(proveedores, text=texto, bg="#FFD700").grid(row=i, column=0, padx=10, pady=5, sticky="w")
        entradas[texto] = Tk.Entry(proveedores, bg="#000000", fg="#FFD700")  
    ...
    for i, (texto, cmd) in enumerate(botones):
        Tk.Button(proveedores, text=texto, width=12, command=cmd, bg="#000000", fg="#FFD700").grid(row=4, column=i, padx=10, pady=10)
    ...
    Tk.Button(proveedores, text="Regresar al Menú", width=20, command=lambda: [proveedores.destroy(), menu.abrir_menu()], bg="#000000", fg="#FFD700").grid(row=7, column=0, columnspan=4, pady=10)
